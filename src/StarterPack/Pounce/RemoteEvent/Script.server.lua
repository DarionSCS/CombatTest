script.Parent.OnServerEvent:Connect(function(plr,direction,mouseaim)
	...
	local flh = game.ReplicatedStorage.Hitboxes.hbox2:Clone()
	flh.Parent = workspace
	flh.CFrame = hrp.CFrame *CFrame.new(0,0,-8)
	...
	local thrust = game.ReplicatedStorage.FX.Pounce.Attachment:Clone()
	thrust.Parent = hrp
	game.Debris:AddItem(thrust,5)
	spawn(function()
	for i, v in pairs(thrust:GetChildren()) do
	...
	flh.Touched:Connect(function(hit, Player)
		if hit.Parent:FindFirstChild("Humanoid") and hit.Parent.Name ~= plr.Name then
			if not hit.Parent.Humanoid:FindFirstChild(plr.Name) then
				...
				misc.StrongKnockback(hit.Parent.HumanoidRootPart, 25, 55, .1, hit.Parent.HumanoidRootPart)
